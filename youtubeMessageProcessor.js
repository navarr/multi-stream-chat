class YoutubeMessageProcessor {
    ...
    replaceYoutubeEmoji(messageHtml) {
        for(let emojiIndex in this.globalEmoji) {
            const emoji = this.globalEmoji[emojiIndex]
            for(let shortcutIndex in emoji.shortcuts) {
                const emojiCode = emoji.shortcuts[shortcutIndex]
                messageHtml = messageHtml.replaceAll(emojiCode, this.buildYoutubeEmojiImageTag(emoji))
            }
        }
        return messageHtml
    }

    buildYoutubeEmojiImageTag(emoji) {
        const baseImage = emoji.image.thumbnails[0].url
        const srcsetArray = []
        for(let thumbIndex in emoji.image.thumbnails) {
            const thumbnail = emoji.image.thumbnails[thumbIndex]
            srcsetArray.push(`${thumbnail.url} ${thumbnail.width}w`)
        }
        return `<img src="${baseImage}" srcset="${srcsetArray.join(', ')}" alt="${emoji.image.accessibility.accessibilityData.label}">`
    }

    async instantiateBttvEmotes(channelId) {
        const bttvResult = await fetch('https://api.betterttv.net/3/cached/users/youtube/' + channelId);
        if (bttvResult.ok) {
    ...
    addBttvEmotes(emotes) {
        for(let emoteIndex in emotes) {
            ...
            const emoteImageUrl = `https://cdn.bettertvv.net/emote/${emotes[emoteIndex].id}/2x`
...
module.exports = {YoutubeMessageProcessor}